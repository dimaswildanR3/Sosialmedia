...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Edit File Konten</li>
    </ol>
</nav>
...
<div class="card shadow p-3 mb-5 bg-white rounded">
    <form action="{{ route('file_kontens.update', $file->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        ...
        <div class="form-group mb-3">
            ...
            <select name="jadwal_konten_id" id="jadwal_konten_id" class="form-control @error('jadwal_konten_id') is-invalid @enderror">
                ...
                <option value="{{ $jadwal->id }}" {{ old('jadwal_konten_id', $file->id_jadwal) == $jadwal->id ? 'selected' : '' }}>
    {{ $jadwal->judul_konten }}
</option>
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" name="nama_file" id="nama_file" class="form-control @error('nama_file') is-invalid @enderror" value="{{ old('nama_file', $file->nama_file) }}">
        ...
        <div class="form-group mb-3">
            ...
            <select name="tipe_file" id="tipe_file" class="form-control @error('tipe_file') is-invalid @enderror">
                ...
                <option value="image" {{ old('tipe_file', $file->tipe_file) == 'image' ? 'selected' : '' }}>Gambar (JPG, PNG)</option>
                <option value="pdf" {{ old('tipe_file', $file->tipe_file) == 'pdf' ? 'selected' : '' }}>PDF</option>
                <!-- <option value="video" {{ old('tipe_file', $file->tipe_file) == 'video' ? 'selected' : '' }}>Video (MP4)</option> -->
            </select>
        ...
        <div class="form-group mb-3">
            ...
            <div id="preview-container" class="mt-2">
                @if($file->tipe_file == 'image' && $file->url_file)
                    <p>File saat ini:</p>
                    <img id="preview-image" src="{{ asset('storage/' . $file->url_file) }}" alt="Preview Gambar" style="max-width: 250px; max-height: 150px; object-fit: contain; border: 1px solid #ddd; padding: 4px;">
                @elseif($file->url_file)
                    <p id="preview-file-name">File saat ini: {{ basename($file->url_file) }}</p>
                @else
...
<script>
    ...
    // Set accept file input saat halaman load berdasarkan tipe file yg sudah ada (edit)
